...
- `backend/` Spring Boot 3 (Java 17) + Spring Cloud Alibaba（Nacos 注册发现/配置中心）+ MyBatis-Plus + MySQL(默认，开发库 192.168.1.38:3306，root/root，库名 zhishu)/H2 内存库(`SPRING_PROFILES_ACTIVE=h2`) + JWT；表结构与种子数据在 `backend/src/main/resources/db/{schema,data}.sql`。视频经外键关联 category（类别）/blogger（作者），标签走 tag + video_tag。视频源默认 nas 模式：NAS（WD My Cloud EX2 Ultra，192.168.1.2，共享 zhishu-video）经 SMB 挂载读取（`bash scripts/mount-nas.sh`，凭据在仓库外 `~/.config/zhishu/nas.env`；`install-nas-autostart.sh` 装开机自动挂载）。MySQL 建库建表用 `bash scripts/init-mysql.sh`（应用自身不自动建表）。Nacos 开发环境在局域网 `192.168.1.38:8848`（账号 nacos/nacos，分组 `ZHISHU_GROUP`），不可用时不阻断启动；本机自建 Nacos 用 `deploy/nacos/docker-compose.yml`。线上视频源将切阿里云 OSS，media_key 保持存储中立、新增 oss Resolver 即可
