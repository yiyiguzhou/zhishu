...
4. **提交信息**用中文，格式：
   ...
   ```
5. **默认开发分支是 `v1`**（GitHub 仓库默认分支同为 v1；`main` 只作发布/稳定分支，日常不直接提交）。新会话先 `git branch --show-current` 确认在 v1 上。较大的改动（新功能、跨端改动、有风险的重构）从 v1 开分支 `feat/<名称>`，完成验证后合回 v1；小修复可直接提交 v1。绝不在一条提交里混入"半成品"。
